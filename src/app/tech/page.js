...
export default function Home() {
  ...
  const fetchArticles = useCallback(async (pageNumber) => {
    try {
      setIsLoading(true);
      const response = await fetch(`/api/rss/ai?page=${pageNumber}&tag=ai`);
      if (!response.ok) {
      ...
      setArticles(prevArticles => {
        ...
        const uniqueArticles = Array.from(new Set(combinedArticles.map(a => a.id)))
          ...
          .filter(article => article.tags && article.tags.includes('ai'));
  ...
  return (
    <div className="container mx-auto p-4">
        ...
        {isLoading && (
          <>
            {[1, 2, 3].map((item) => (
              <div key={item} className="border p-4 rounded-lg mb-4 flex animate-pulse">
